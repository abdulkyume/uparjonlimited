...
export class OrderComponent implements OnInit, OnDestroy {
      ...
      .subscribe({
        next: (res: any) => {
          ...
          res.data.content.map((merchant: any) => {
            merchantdetailsList.push({ id: merchant.id, customtext: merchant.name + " - " + merchant.phoneNumber })
          })
        },
        error: (err: any) => {
          console.error(err);
  ...
  submitOrder() {
    ...
    if (this.orderForm.invalid || this.orderForm.controls["orderDetails"].value?.length < 1) {
      return;
    }
    if (this.orderForm.controls["id"].value) {
      this.loader = true;
      this.orderForm.controls["orderDetails"].value.map((d: any) => {
        let ids = this.dropdownList1.filter((idsd: any) => idsd.id === d.itemId);
        d.amount = d.quantity * ids[0].customtext.split('-')[1];
      });
      this.orderForm.controls["orderDetails"].setValue(this.slubdata().value.concat(this.deletedData));
      this.orderService.updateOrder(this.orderForm.value).pipe(takeUntil(this.ngUnsubscribe))
        .subscribe({
          next: (res: any) => {
            if (res.isSuccess) {
      ...
      this.orderService.addOrder(this.orderForm.value).pipe(takeUntil(this.ngUnsubscribe))
        .subscribe({
          next: (res: any) => {
            if (res.isSuccess) {
              this.successmsg(res.message)
  ...
  editOrder(data: any) {
    ...
    this.selectedItems = this.dropdownList.filter((dta: any) => dta.id == data.merchantId);
  ...
  getOrderDetail(date: string, orderId: string) {
    this.loader = true;
    this.orderService.getAllOrderDetail(date, date, this.userid, orderId).pipe(takeUntil(this.ngUnsubscribe))
      .subscribe({
        next: (res: any) => {
          if (res.isSuccess) {
            let data = res.data;
